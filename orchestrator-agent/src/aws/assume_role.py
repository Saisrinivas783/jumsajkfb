...
client (Bedrock, ...) gets STS role assumption with correct concurrent-refresh
semantics without hand-rolling expiry tracking: botocore's own
advisory/mandatory refresh windows and internal locking already handle that
(non-blocking refresh attempts outside the mandatory window, automatic
...
class AssumedRoleClientFactory:
    ...
    def refresh_metadata(self) -> dict[str, str]:
        ...
        try:
            ...
            raise CredentialsError(f"AssumeRole failed: {error_code}") from exc
        except Exception as exc:
            # Log the full exception server-side only; the raised message can
            # reach an HTTP response body via the orchestrator error handler,
            # so it must carry just the exception's class name, never its raw
            # text (which could embed hostnames, paths, or other detail).
            logger.error("AssumeRole failed for %s: %s", self.method, exc)
            raise CredentialsError(
                f"AssumeRole failed: {type(exc).__name__}"
            ) from exc
